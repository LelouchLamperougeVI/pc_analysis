...
axis square
xlabel('Exposure Correlations')
ylabel('Rest 1 (pre-exposure) Correlations')
title(['Corr Coef = ' num2str(PreEx) ' R^{2} = ' num2str(PreEx^2) ' p = ' num2str(p1)]);
xlim([-0.2 0.8]);
ylim([-0.2 0.8]);
...
title(['Partial Corr Coef = ' num2str(PostEx) ' R^{2} = ' num2str(PostEx^2) ' p = ' num2str(p2)]);
xlim([-0.2 0.8]);
ylim([-0.2 0.8]);