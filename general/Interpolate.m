...
if ~isdvector(timestamps),
	error('Incorrect timestamps - should be a vector');
end
...
for j = 1:2:length(varargin),
	if ~ischar(varargin{j}),
		error(['Parameter ' num2str(j+2) ' is not a property']);
	end
	switch(lower(varargin{j})),
		...
		case 'trim',
			...
			if ~isstring(trim,'on','off'),
				error('Incorrect value for property ''trim''');
			end
		case 'maxgap',
			maxGap = varargin{j+1};
		...
		otherwise,
			error(['Unknown property ''' num2str(varargin{j}) '''']);
	end
end
